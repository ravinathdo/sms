...
//validation rules
    ...
    public function getBrokerFunndsForUser($param) {
        /*
          SELECT broker_fund.*,brokercompany.name
          FROM broker_fund
          INNER JOIN brokercompany
          ON broker_fund.brokercomid = brokercompany.brokercomid
          WHERE broker_fund.userid = 10
         */
        $this->db->select('broker_fund.*,brokercompany.name');
        $this->db->from('broker_fund');
        $this->db->join('brokercompany', 'broker_fund.brokercomid = brokercompany.brokercomid');
        $where = 'broker_fund.userid = ' . $param;
        $this->db->where($where);
        $query = $this->db->get();
